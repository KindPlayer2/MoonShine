...
public class Bandit : MonoBehaviour
{
    ...
    IEnumerator StopAndShoot()
    {
        ...
        if (canShoot)
        {
            ...
            if(gameObject.name != "Bandit" && !playa.isInside)
            {
            ...
            // Spawn 8 bullets in different directions
            for (int i = 0; i < 8; i++)
            {
                SpawnBullet(i * 45f); // Each bullet will be 45 degrees apart
    ...
    {
        // Calculate the direction for the bullet based on the angle
        Vector2 bulletDirection = new Vector2(Mathf.Cos(Mathf.Deg2Rad * angle), Mathf.Sin(Mathf.Deg2Rad * angle)).normalized;
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (other.CompareTag("Bullet"))
        {
            ...
            else//IT'LL TAKE MORE THAN THAT TO BEAT ME HAHAAHA
            {
                health--;
            }
        }
