...
public class Sheriff : MonoBehaviour
{
    ...
    private bool IsOriginal()
    {
        ...
        return gameObject.name == "Sheriff";
    }
    ...
    IEnumerator StopAndShoot()
    {
        ...
        if (canShoot)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                float angleStep = 15f; // Step size between each bullet

                for (float currentAngle = lowerRange; currentAngle <= upperRange; currentAngle += angleStep)
                {
                    // Set shooting direction based on the calculated angle
                    shootingDirection = new Vector2(Mathf.Cos(currentAngle * Mathf.Deg2Rad), Mathf.Sin(currentAngle * Mathf.Deg2Rad)).normalized;

                    animator.SetTrigger("Shoot");

                    if(gameObject.name != "Sheriff"  && !playa.isInside)
                    {
